...
if ($first == "single_applicant") {
    ...
    if ($applicant_type === "applicant") {
        ...
        if (!empty($student_id)) {
            ...
            $get_applicant_info->application_no = random_str(4, '12345634608974');
            ...
            if ($application_sent) {
                ...
                $redirectlink = "";
                $redirectlink = "university-details/" . $get_applicant_info->university_id;
                $message = 'Your Application was Recieved ' . $kd->config->name;
                $update_data = array(
                    'UNIVERSITY' =>  GetuniversityByID($get_applicant_info->university_id),
                    'PROGRAM' => GetProgramByID($get_applicant_info->program_id),
                    'TRACKING_CODE' =>  $referencetrack,
                    'URL' => UrlLink('status'),
                    'NAME' =>  $get_applicant_info->first_name . ' ' .  $get_applicant_info->middle_name . ' ' . $get_applicant_info->last_name,
                );
                $send_email_data = array(
                    'from_email' => $kd->config->email,
                    'from_name' => $kd->config->name,
                    'to_email' => $get_applicant_info->email,
                    'to_name' => $get_applicant_info->first_name . ' ' .  $get_applicant_info->middle_name . ' ' . $get_applicant_info->last_name,
                    'subject' => 'Application Recieved',
                    'charSet' => 'UTF-8',
                    'message_body' => LoadPage('email/application_sent', $update_data),
                    'is_html' => true
                ...
                $notif_data = array(
                    ...
                    'application_id' => '0',
                    'type' => 'new_application',
                    'text' => ' Applied at <b>' . GetuniversityByID($get_applicant_info->university_id) . '</b> in' . GetProgramByID($get_applicant_info->program_id) . '</b>',
                    'url' => ('/admin-cp/view-application?id=' . $uniqid),
                    'time' => time(),
                    'target' => 'all',
                );
